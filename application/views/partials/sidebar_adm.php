...
<div id="sidebar-wrapper" class="d-flex flex-column">
    ...
    <ul class="nav nav-pills flex-column px-3 mb-auto" style="overflow-y: auto;">
        ...
        <li class="nav-item">
            <a href="<?= base_url('admin/mahasiswa') ?>" class="nav-link <?= ($this->uri->segment(2) == 'mahasiswa') ? 'active' : '' ?> d-flex align-items-center gap-3">
        ...
        <li class="nav-item">
            <a href="<?= base_url('admin/mitra') ?>" class="nav-link <?= ($this->uri->segment(2) == 'mitra') ? 'active' : '' ?> d-flex align-items-center gap-3">
...
<div id="page-content-wrapper">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Script Theme Toggle (Opsional jika perlu AJAX)
            const themeToggle = document.getElementById('themeToggle');
            if(themeToggle) {
                themeToggle.addEventListener('change', function() {
                    // Masukkan logika AJAX ke Controller untuk simpan session theme disini jika perlu
                    location.reload(); // Sementara reload sederhana
                });
            }
        });
    </script>